...
const {
    ...
    sendNewCode,
    verifyCode,
} = require(Features.DB_SERVICES ? './middleware/2fa' : './middleware/2fa.legacy');
...
} = require(Features.DB_SERVICES ? './middleware/fundedAccount' : './middleware/fundedAccount.legacy');
router.post(
    '/fundedAccount',
...
router.post(
    ...
    Features.DB_SERVICES ? (ctx, next) => verifyAccountExists(ctx, next, ctx.request.body) : createWithSequelizeAcccountMiddleware('body'),
    clearFundedAccountNeedsDeposit,
);
...
router.get(
    ...
    Features.DB_SERVICES ? (ctx, next) => verifyAccountExists(ctx, next, ctx.params) : createWithSequelizeAcccountMiddleware('params'),
    async (ctx) => {
        if (!Features.DB_SERVICES) {
...
router.post('/account/validateSecurityCode',
    checkCreateableRecoveryMethod,
    checkAccountOwnership,
    Features.DB_SERVICES ? completeRecoveryValidation() : completeRecoveryValidation_legacy(),
);

router.post('/account/validateSecurityCodeForTempAccount',
    checkCreateableRecoveryMethod,
    createCheckAccountDoesNotExistMiddleware({ source: 'body', fieldName: 'accountId' }),
    Features.DB_SERVICES ? completeRecoveryValidation({ isNew: true }) : completeRecoveryValidation_legacy({ isNew: true })
);
